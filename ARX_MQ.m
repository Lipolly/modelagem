...
%% Definição do Sinal de Entrada : Excitação PRBS
N=256;
u=prbs(N,8,2)';
Ts=0.1;
t=[0:Ts:(N-1)*Ts];
...
ky=2;               % Número de atrasos na saída
ku=2;               % Número de atrasos na entrada

%% Identificação usando função IDENT
...
%% Simulação Modelo para uma entrada em degrau 
...
subplot(221)
plot(t,u); title('Entrada PRBS: u'); 
subplot(223)
plot(t,y); title('Saída PRBS: y');
subplot(222)
plot(t,u_simulacao); title('Entrada Degrau'); 
subplot(224)
y_real=step(G,t); 
y_real_ident=step(Gident,t); 
plot(t,y_real,t,y_modelo,t,y_real_ident);hold on;title('Resposta ao Degrau: Real x Modelo Identificado'); 
legend('Real','Identificado','Ident')
